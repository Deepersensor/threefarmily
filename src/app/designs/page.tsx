...
import { designItems } from '@/data/marketplace';
import BuyItemButton from '@/components/marketplace/BuyItemButton';

export default function DesignsMarketplace() {
  const [filter, setFilter] = useState('all');
  const [sortBy, setSortBy] = useState('newest');
  
  // Filter items by price range
  const getFilteredItems = () => {
    if (filter === 'all') return designItems;
    if (filter === 'low') return designItems.filter(item => parseFloat(item.price) < 1.0);
    if (filter === 'medium') return designItems.filter(item => parseFloat(item.price) >= 1.0 && parseFloat(item.price) <= 2.0);
    if (filter === 'high') return designItems.filter(item => parseFloat(item.price) > 2.0);
    return designItems;
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 pb-24 md:pb-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2 md:mb-0">Farm Designs</h1>
          <p className="text-gray-600 dark:text-gray-400">Architectural plans and blueprints for modern farming</p>
        </div>
      ...
      <div className="mb-8 flex flex-wrap items-center gap-4 mt-6">
        <div className="flex items-center">
          ...
          >
            ...
            <option value="low">Below 1 ETH</option>
            <option value="medium">1-2 ETH</option>
            <option value="high">Above 2 ETH</option>
          </select>
        </div>
