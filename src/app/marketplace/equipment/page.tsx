...
export default function EquipmentMarketplace() {
  ...
  const getFilteredItems = () => {
    if (filter === 'all') return equipmentItems;
    if (filter === 'low') return equipmentItems.filter(item => parseFloat(item.price) < 1.0);
    if (filter === 'medium') return equipmentItems.filter(item => parseFloat(item.price) >= 1.0 && parseFloat(item.price) <= 3.0);
    if (filter === 'high') return equipmentItems.filter(item => parseFloat(item.price) > 3.0);
  ...
  return (
    <div className="container mx-auto px-4 py-8 pb-24 md:pb-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-2 md:mb-0">Farm Equipment</h1>
          <p className="text-gray-600 dark:text-gray-400">Find the best equipment for your farm</p>
        </div>
      ...
      <div className="mb-8 flex flex-wrap items-center gap-4 mt-6">
        <div className="flex items-center">
          ...
          >
            ...
            <option value="high">Above 3 ETH</option>
          </select>
        </div>
